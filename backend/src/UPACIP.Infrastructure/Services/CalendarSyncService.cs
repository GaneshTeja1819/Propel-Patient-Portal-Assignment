...
namespace UPACIP.Infrastructure.Services;
...
public sealed class CalendarSyncService : ICalendarSyncService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var redirectUri = GetRequiredEnvOrConfig("CALENDAR_REDIRECT_URI", "Calendar:RedirectUri");
        ...
        await using var tokenJson = await response.Content.ReadAsStreamAsync(cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var outlookRequest = new HttpRequestMessage(HttpMethod.Post, OutlookEventsUrl)
        {
            Headers = { Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", accessToken) },
            Content = new StringContent(JsonSerializer.Serialize(outlookBody), Encoding.UTF8, "application/json"),
        };

        var outlookResponse = await _httpClient.SendAsync(outlookRequest, cancellationToken);
        outlookResponse.EnsureSuccessStatusCode();

        await using var outlookStream = await outlookResponse.Content.ReadAsStreamAsync(cancellationToken);
        using var outlookDoc = await JsonDocument.ParseAsync(outlookStream, cancellationToken: cancellationToken);
        return outlookDoc.RootElement.GetProperty("id").GetString()!;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var outlookBody = BuildOutlookEventPayload(appointment);
        var outlookUrl = $"{OutlookEventsUrl}/{Uri.EscapeDataString(eventId)}";
        var outlookRequest = new HttpRequestMessage(new HttpMethod("PATCH"), outlookUrl)
        {
            Headers = { Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", accessToken) },
            Content = new StringContent(JsonSerializer.Serialize(outlookBody), Encoding.UTF8, "application/json"),
        };
