...
- [x] `admin_read_role` PostgreSQL role provisioned with SELECT on `audit.*` only (EP-DATA dependency confirmed)
- [x] `AuditReadDbContext` registered with `admin_read_role` connection string from GitHub Secrets (AC-006)
- [x] `AdminAuditLogController` decorated with `[Authorize(Policy = "AdminPolicy")]`; Staff/Patient → HTTP 403 (AC-007)
- [x] GET list: all filter params applied as AND-chained WHERE clauses; ordered newest-first; paginated (AC-001, AC-002)
- [x] GET stats: scoped to UTC today; returns `EventsToday`, `UniqueUsers`, `PhiAccessEvents`, `FailedAttempts` (AC-001)
- [x] GET export: CSV stream via `IAsyncEnumerable`; no PHI values in CSV; `AUDIT_LOG_EXPORTED` written (AC-005)
- [x] `AUDIT_LOG_VIEWED` written per list request (fire-and-forget; does not block response) (AC-008)
- [x] `[ResponseCache(NoStore = true)]` on all endpoints (HIPAA compliance)
- [x] No `[HttpPost/Put/Patch/Delete]` on controller; HTTP 405 for all non-GET verbs (AC-006)
- [x] `PayloadJson` PHI masking: unit tested for all `phiTagged` fields (AC-004)
- [x] Empty result returns HTTP 200 `{ data: [], total: 0 }` not HTTP 404 (edge case)
