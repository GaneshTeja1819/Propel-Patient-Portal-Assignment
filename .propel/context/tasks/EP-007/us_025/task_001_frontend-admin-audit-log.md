...
- [ ] `AdminAuditLogPage` guarded with `<RequireRole role="Admin" />`; redirects non-Admin (AC-007)
- [ ] HIPAA notice banner always visible; not dismissible (AC-004)
- [ ] `AuditStatStrip`: 4 cards populated from stats endpoint; failed-attempts error colour when > 0 (AC-001)
- [ ] `AuditFilterBar`: all 5 filter controls; debounced search (300 ms); Reset restores defaults (AC-002)
- [ ] `AuditLogTable`: paginated 15 rows/page; action badge colours correct; role/status badges; PHI indicator on applicable events (AC-001, AC-004)
- [ ] `AuditEventDetailRow`: event ID + ISO timestamp + actor + payload; PHI patient ID labelled; immutable notice (AC-003)
- [ ] Single-expand constraint: opening new row closes previous (AC-003)
- [ ] Escape key closes all expanded rows (AC-003)
- [ ] `exportCSV`: download triggered; toast shown; button not permanently disabled (AC-005)
- [ ] No edit/delete/modify controls anywhere on the page (AC-006)
- [ ] 401 on session expiry → redirect to `/login?redirect=/admin/audit-log` (edge case)
- [ ] Empty state "No events match the selected filters" when result count = 0 (edge case)
